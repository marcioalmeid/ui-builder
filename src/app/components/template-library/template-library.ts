...
import { scenarioGuide } from '../../catalog/scenario-seeds';
...
export class TemplateLibrary {
  ...
  selectedSpikeId = signal('s1-baseline');

  spikePacks = computed(() => this.scenarios.listPacks());
  loadedSpikeId = computed(() => this.scenarios.currentPackId());
  ...
  loadSelectedScenario() {
    const pack = this.scenarios.installScenario(this.selectedSpikeId());
    if (!pack) {
      window.alert('Unknown spike scenario.');
      return;
    }
    void this.router.navigate(['/builder', pack.template.id]);
  }

  guideFor(template: TaskTemplate) {
    return scenarioGuide(template.name);
  }

  seedJobId(template: TaskTemplate): string | null {
    if (template.name.startsWith('[S0]')) return null;
    return `${template.id}-seed-job`;
  }
}
